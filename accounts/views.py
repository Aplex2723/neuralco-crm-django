...
from common.models import Attachments, Comment, Profile, Org
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.profile.role != "ADMIN" and not self.request.user.is_superuser:
            queryset = queryset.filter(
                Q(created_by=profile) | Q(assigned_to=profile)
            ).distinct()
...
class AccountDetailView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        if serializer.is_valid():
            ...
            if (
                ...
                if not (
                    (profile == account_object.created_by)
                    or (profile in account_object.assigned_to.all())
                ):
                    return Response(
                        {
                            "error": True,
    ...
    def delete(self, request, pk, format=None):
        ...
        if profile.role != "ADMIN" and not self.request.user.is_superuser:
            if profile != self.object.created_by:
                return Response(
                    {
                        "error": True,
                        "errors": "You do not have Permission to perform this action",
    ...
    def get(self, request, pk, format=None):
        ...
        if profile.role != "ADMIN" and not self.request.user.is_superuser:
            if not (
                (profile == self.account.created_by)
                or (profile in self.account.assigned_to.all())
            ):
                return Response(
                    {
                        "error": True,
        ...
        elif profile != self.account.created_by:
    ...
    def post(self, request, pk, **kwargs):
        ...
        if profile.role != "ADMIN" and not self.request.user.is_superuser:
            if not (
                (profile == self.account_obj.created_by)
                or (profile in self.account_obj.assigned_to.all())
            ):
                return Response(
                    {
                        "error": True,
