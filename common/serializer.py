...
from common.models import (Address, APISettings, Attachments, Comment,
                           Document, Org, Profile, User, Subscription)
...
class OrgProfileCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, name):
        if bool(re.search(r"[~\!_.@#\$%\^&\*\ \(\)\+{}\":;'/\[\]]", name)):
            raise serializers.ValidationError({ 'org_special_chars_error': "Organization name should not contain any special characters"})
        if Org.objects.filter(name=name).exists():
            raise serializers.ValidationError({
                'org_name_error': "Organization already exists with this name"
            })
        return name
