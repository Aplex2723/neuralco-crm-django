...
from common.utils import COUNTRIES, ROLES, PLAN_CHOICES
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    profile_pic = models.FileField(
        max_length=1000, upload_to=img_url, null=True, blank=True, verbose_name='Profile Pic'
    )
    activation_key = models.CharField(max_length=150, null=True, blank=True)
    key_expires = models.DateTimeField(null=True, blank=True)
    skype_ID = models.CharField(max_length=50, null=True)
...
class Subscription(models.Model):
    ...
    plan = models.CharField(choices=PLAN_CHOICES, max_length=15, default='basic')
    ...
    def __str__(self):
        return f"{self.user.first_name} - {self.plan}{' (Annual)' if self.is_annual else ''}"
