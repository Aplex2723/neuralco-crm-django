...
urlpatterns = [
    ...
    # GoogleLoginView    path("auth/google/", views.GoogleLoginView.as_view()),
    ...
    path(
        "auth/activate_user/<str:uid>/<str:token>/<str:activation_key>/",
        views.ActivateUserView.as_view(),
    ),
    path("auth/resend-activation-link/", views.ResendActivationLinkView.as_view()),
    path("users/subscription/", views.SubscriptionView.as_view()),
